...
type Scheduler struct {
	...
	TaskId int		`orm:"column(task_id);type(int)" json:"task_id"`
	// 状态：wait 等待执行/executing 执行中/suspend 中断执行/ failure 执行失败/ success 执行成功 .
	Status        string    `orm:"column(status);default(wait)" json:"status"`
	...
	Data          string    `orm:"type(text);column(data)" json:"-"`                                   //数据
	LogContent    string    `orm:"column(log_content);type(text);" json:"log_content"`
	// 执行方式：0 自动触发 / 1 重试触发 / 2 手动执行 .
	ExecuteType   int       `orm:"column(execute_type);type(int);default(0)" json:"execute_type"`
}
